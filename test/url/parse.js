...
describe('S3/url/parse', function() {
    it('parse(absoluteUrl)', function () {
        ...
        expect(url.url).toBe('https://example.com:80/books/123/?a=1&b=3#hashid');
        ...
        expect(url.port).toBe('80');
        expect(url.url).toBe('http://example.com/books/123/?a=1&b=3#hashid');
    });
    ...
    it('parse(relativeUrl)', function () {
        ...
        expect(url.query).toBe('?a=1&b=3');
        expect(url.url).toBe('http://localhost:9876/books/123/?a=1&b=3#hashid');
    });
});
